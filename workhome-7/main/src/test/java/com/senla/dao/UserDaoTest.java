...
//        Assertions.assertTrue(allUsers.size() >= defaultPrivilegeCode.size());
        Assertions.assertEquals(allUsers.size(), 5);
        Assertions.assertEquals(defaultPrivilegeCode.size(), 4);
    }

    @Test
    void testFindUserWithSpecifiacation(){
        String email = "max@gmail.com";
        ...
        List<User> users = userDao.findUserBySpecification(spec);
        ...
        Assertions.assertEquals(user.getEmail(), email);
        Assertions.assertTrue(user.getRegistrationDate().isAfter(from));
        Assertions.assertTrue(user.getRegistrationDate().isBefore(to));
    }
